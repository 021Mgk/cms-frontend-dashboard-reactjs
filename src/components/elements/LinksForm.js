  ...
  return (
    <>
      {Object.keys(message).length !== 0 ? (
        <div
          className={`alert ${
            message.type === 1 ? 'alert-success' : 'alert-danger'
          }`}
        >
          {' '}
          {message.msg}{' '}
        ...
        onSubmit={async (values, { setSubmitting }) => {
          ...
          if (actionType === 'POST') {
            ...
            if (resp.ok) {
              ...
              if (resp.status === 403) {
                ...
                setTimeout(() => alert('sss'), 3000);
                setLogin(true);
              }
              setMessage({
                msg: 'لطفا مجدد تلاش کنید مشکلی در ثبت فرم وجود دارد.',
                type: 2,
              });
              console.log(resp);
            }
          ...
          if (actionType === 'PUT') {
            const resp = await fetch(
              `http://localhost:8080/api/v1/links/${editData?.id}`,
              {
                method: 'PUT',
                credentials: 'include',
                headers: {
            ...
            if (resp.ok) {
              ...
              setMessage({
                msg: 'لطفا مجدد تلاش کنید مشکلی در بروزرسانی فرم وجود دارد.',
                type: 2,
              });
            }
          }
          ...
          >
            ...
            <div className='field__holder without__alarm'>
              ...
              <input
                ...
                onBlur={handleBlur}
              />
              {(iconPreview || values.icon) && (
                <img
                  alt='image perview'
                  src={iconPreview ? iconPreview : values.icon}
            ...
            <div className='btn_holder'>
              <input
                ...
                value={actionType === 'POST' ? 'ثبت جدید' : 'به روز رسانی'}
              />
            </div>
          </form>
        )}
...
export default connect()(LinksForm);
